...
<ul id="detail_header_action_menu" class="clickMenu fancymenu" name ><li class="sugar_action_button" >{if $bean->aclAccess("edit")}<input title="{$APP.LBL_EDIT_BUTTON_TITLE}" accessKey="{$APP.LBL_EDIT_BUTTON_KEY}" class="button primary" onclick="var _form = document.getElementById('formDetailView'); _form.return_module.value='pos_SponsorContacts'; _form.return_action.value='DetailView'; _form.return_id.value='{$id}'; _form.action.value='EditView';SUGAR.ajaxUI.submitForm(_form);" type="button" name="Edit" id="edit_button" value="{$APP.LBL_EDIT_BUTTON_LABEL}">{/if} <ul id class="subnav" ><li>{if $bean->aclAccess("edit")}<input title="{$APP.LBL_DUPLICATE_BUTTON_TITLE}" accessKey="{$APP.LBL_DUPLICATE_BUTTON_KEY}" class="button" onclick="var _form = document.getElementById('formDetailView'); _form.return_module.value='pos_SponsorContacts'; _form.return_action.value='DetailView'; _form.isDuplicate.value=true; _form.action.value='EditView'; _form.return_id.value='{$id}';SUGAR.ajaxUI.submitForm(_form);" type="button" name="Duplicate" value="{$APP.LBL_DUPLICATE_BUTTON_LABEL}" id="duplicate_button">{/if} </li><li>{if $bean->aclAccess("delete")}<input title="{$APP.LBL_DELETE_BUTTON_TITLE}" accessKey="{$APP.LBL_DELETE_BUTTON_KEY}" class="button" onclick="var _form = document.getElementById('formDetailView'); _form.return_module.value='pos_SponsorContacts'; _form.return_action.value='ListView'; _form.action.value='Delete'; if(confirm('{$APP.NTC_DELETE_CONFIRMATION}')) SUGAR.ajaxUI.submitForm(_form);" type="submit" name="Delete" value="{$APP.LBL_DELETE_BUTTON_LABEL}" id="delete_button">{/if} </li><li>{if $bean->aclAccess("edit") && $bean->aclAccess("delete")}<input title="{$APP.LBL_DUP_MERGE}" class="button" onclick="var _form = document.getElementById('formDetailView'); _form.return_module.value='pos_SponsorContacts'; _form.return_action.value='DetailView'; _form.return_id.value='{$id}'; _form.action.value='Step1'; _form.module.value='MergeRecords';SUGAR.ajaxUI.submitForm(_form);" type="button" name="Merge" value="{$APP.LBL_DUP_MERGE}" id="merge_duplicate_button">{/if} </li><li>{if $bean->aclAccess("detail")}{if !empty($fields.id.value) && $isAuditEnabled}<input id="btn_view_change_log" title="{$APP.LNK_VIEW_CHANGE_LOG}" class="button" onclick='open_popup("Audit", "600", "400", "&record={$fields.id.value}&module_name=pos_SponsorContacts", true, false,  {ldelim} "call_back_function":"set_return","form_name":"EditView","field_to_name_array":[] {rdelim} ); return false;' type="button" value="{$APP.LNK_VIEW_CHANGE_LOG}">{/if}{/if}</li></ul></li></ul>
</div>
</td>
...
{capture name="label" assign="label"}{sugar_translate label='LBL_NAME' module='pos_SponsorContacts'}{/capture}
...
{capture name="label" assign="label"}{sugar_translate label='LBL_HOME_PHONE' module='pos_SponsorContacts'}{/capture}
{$label|strip_semicolon}:
{/if}
</td>
<td width='37.5%'  class="phone">
...
{capture name="label" assign="label"}{sugar_translate label='LBL_OTHER_PHONE' module='pos_SponsorContacts'}{/capture}
{$label|strip_semicolon}:
{/if}
</td>
<td width='37.5%'  class="phone">
...
{capture name="label" assign="label"}{sugar_translate label='LBL_DATE_ENTERED' module='pos_SponsorContacts'}{/capture}
{$label|strip_semicolon}:
{/if}
</td>
<td width='37.5%'  >
...
{capture name="label" assign="label"}{sugar_translate label='LBL_DATE_MODIFIED' module='pos_SponsorContacts'}{/capture}
{$label|strip_semicolon}:
{/if}
</td>
<td width='37.5%'  >
...
{capture name="label" assign="label"}{sugar_translate label='LBL_DO_NOT_CALL' module='pos_SponsorContacts'}{/capture}
{$label|strip_semicolon}:
{/if}
</td>
<td width='37.5%'  >
...
{capture name="label" assign="label"}{sugar_translate label='LBL_PRIMARY_ADDRESS' module='pos_SponsorContacts'}{/capture}
{$label|strip_semicolon}:
{/if}
</td>
<td width='37.5%'  >
...
{capture name="label" assign="label"}{sugar_translate label='LBL_ALT_ADDRESS' module='pos_SponsorContacts'}{/capture}
{$label|strip_semicolon}:
{/if}
</td>
<td width='37.5%'  >
...
{capture name="label" assign="label"}{sugar_translate label='LBL_DESCRIPTION' module='pos_SponsorContacts'}{/capture}
{$label|strip_semicolon}:
{/if}
</td>
<td width='37.5%'  >
...
{counter name="fieldsUsed"}
<td width='12.5%' scope="col">
{if !$fields.pos_sponsorrcontacts_name.hidden}
&nbsp;
{/if}
</td>
<td width='37.5%'  >
